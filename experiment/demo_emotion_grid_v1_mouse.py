...
pics = loadFiles('stimuli_demo', 'png', 'image', win)


picList = list(pics.keys()) #list of picture names
numPics = len(pics.keys()) #how many pictures
...
selectedStim = None #keeps track of what icture selected
            ...
            for i in bbars:
                ...
                if i.contains(tuple_stimPos):
                    ...
                    if tuple(i.pos) in positions:
                        #move to center (15,15) based on positions
                        if tuple_stimPos[0] > 0 and tuple_stimPos[1] > 0:
                            newX = tuple_stimPos[0] - 15
                            newY = tuple_stimPos[1] - 15
...
while True:
    ...
    #press qap to end the task
    if ('q' and 'a' and 'p') in keys:
        break
